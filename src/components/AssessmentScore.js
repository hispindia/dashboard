...
const AssessmentScore = ({ chartId, scoreId }) => {
  ...
  useEffect(() => {
    if(assessmentScore) {
      ...
      if(chartId=="PHC/UPHC" && userOU.id==selectedOU.id) {
        options['plotOptions']['series']['dataLabels']['rotation'] = 280;
        options['plotOptions']['series']['dataLabels']['y'] = -20;
      } else {

        options['plotOptions']['series']['dataLabels']['rotation'] = 0;
        options['plotOptions']['series']['dataLabels']['y'] = 0;
      }
      setOptions(options);
    }
  }, [assessmentScore])
  useEffect(() => {
    ...
    if (orgUnit) {
      ...
      ApiService.getAssesmentScore(
        ...
        setXAxis(axisValues);
        setAssessmentScore(plotValues);
        setQuarter(dataPeriod);
      });
    }
  }, [orgUnit, period]);

  if (!assessmentScore) return null;
  return (
    <>
    <h5 className="fw-bold text-center my-3">
      {chartId} IPA Ex-Ante Assessment Scores (%)
    </h5>
    <hr class="hr" />
