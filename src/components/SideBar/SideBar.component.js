...
const SideBar = ({ data }) => {
  ...
  useEffect(() => {
    if (data) {
      ...
      if(data.programList) dispatch(setHeadList(data.programList.programs));
      ...
      if(data.cateogoryOptionSets) {
        const categories = data.cateogoryOptionSets.options;
      ...
      if (data.subCategoriesGroupList) {
          dispatch(setSubCategoriesList(data.subCategoriesGroupList.optionGroups)); 
      }
    }
  }, [data]);
